...
public class JanelaEstudante implements Initializable {
    ...
    public void initFuncoes(){
        ...
        int codEstudanteProj =0;
        if (proj!=null){
        codEstudanteProj =proj.getCodEstudante();}
        ...
        if(codEstudanteCand==utilizadorIntroduzido.getCodUtilizador()){
            btnCandidatar.setDisable(true);
            btnConvidar.setDisable(true);
        }
        if(codEstudanteProj==utilizadorIntroduzido.getCodUtilizador()){
            btnCandidatar.setDisable(true);
            btnConvidar.setDisable(false);
    ...
    public void handleButtonCandidatarAction(ActionEvent actionEvent) {
        ...
        if(propostaController.getAllPropostasAceitesByCodEdicaoboolean(codEdicao)){
    ...
    public void handleButtonConvidarDocenteAction(ActionEvent actionEvent) {
        ...
        try {
            ...
            boolean criou = conviteController.criarNovoConvite(codProjeto, codEstudante, codDocente);
            ...
            AlertaUI.criarAlerta(Alert.AlertType.ERROR, MainApp.TITULO_APLICACAO, "Não está inserido em nenhum projecto.",
                    e.getMessage()).show();
